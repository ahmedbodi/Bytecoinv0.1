...
WORKINGDIR="/tmp/bytecoin"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="http://downloads.sourceforge.net/project/bytecoin/ByteCoin/bytecoin-0.8.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="http://downloads.sourceforge.net/project/bytecoin/ByteCoin/"
VERSIONPREFIX="bytecoin-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bytecoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[bytecoin-]<version>-[rc[0-9]] (example: bytecoin-0.7.1-rc1)"
   echo "wget output:"
