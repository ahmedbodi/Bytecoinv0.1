...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete ByteCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no ByteCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old ByteCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected ByteCoin 0.7 datadir."
    ;;
  3)
    echo "Detected ByteCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected ByteCoin 0.8 datadir."
    ;;
esac
